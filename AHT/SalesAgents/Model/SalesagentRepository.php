...
class SalesagentRepository implements SalesagentRepositoryInterface
{
    ...
    /** * Prefix of model events names * * @var string */ protected $_eventPrefix = 'saleagent_repository';
    /** * @param \Magento\Framework\Model\Context $context * @param \Magento\Framework\Registry $registry * @param \Magento\Framework\Model\ResourceModel\AbstractResource $resource * @param \Magento\Framework\Data\Collection\AbstractDb $resourceCollection * @param array $data */ public function __construct(ResourceSalesagent $resource, SalesagentFactory $salesagentFactory, StoreManagerInterface $storeManager, DataObjectProcessor $dataObjectProcessor, array $data = [])
    {
    ...
    public function save(\AHT\SalesAgents\Api\Data\SalesAgentInterface $salesagent)
    {
        ...
        try {
            ...
            throw new CouldNotSaveException(__('Could not save the salesagent: %1', $exception->getMessage()), $exception);
        }
        return $salesagent;
    }
    ...
    public function delete(\AHT\SalesAgents\Api\Data\SalesagentInterface $salesagent)
    {
        try {
            ...
            throw new CouldNotDeleteException(__('Could not delete the salesagent: %1', $exception->getMessage()));
        }
        return true;
    }
